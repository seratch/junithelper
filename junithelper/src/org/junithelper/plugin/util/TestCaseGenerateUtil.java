...
public class TestCaseGenerateUtil {
	...
	boolean enabledSupportJMock2 = store
		.getBoolean(STR.Preference.TestMethodGen.USING_JMOCK2);
	boolean enabledSupportEasyMock = store
		.getBoolean(STR.Preference.TestMethodGen.USING_EASYMOCK);
	// enable public method test
	if (enabled) {
	...
	if (enabled) {
		...
		for (String target : targets) {
			...
			if (matcher.find()) {
				...
				String returnTypeFull = getType(matcher
					.group(1));
				// get generics
				...
				for (int i = 0; i < argArrLen; i++) {
				    ...
				    Matcher toGenericsMatcher = Pattern
					    .compile(RXP_GENERICS_PART_GROUP)
					    .matcher(argTypeFull);
				    while (toGenericsMatcher.find()) {
					String[] generics = toGenericsMatcher
						.group().replaceAll("<",
							STR.EMPTY).replaceAll(
							">", STR.EMPTY).split(
							",");
					// convert to java.lang.Object if self
					// class is included
					for (String generic : generics) {
					    generic = getClassInSourceCode(
						    generic, STR.EMPTY,
						    new ArrayList<String>());
