...
public class RegExpTest {
	...
	public void MethodSignatureArea_true() {
		...
		for (int i = 0; i < expectedTrue.length; i++) {
			assertTrue(expectedTrue[i],
					expectedTrue[i].matches(RegExp.MethodSignatureArea));
		}
	}
	...
	public void MethodSignatureArea_false() {
		...
		for (int i = 0; i < expectedFalse.length; i++) {
			assertFalse(expectedFalse[i],
					expectedFalse[i].matches(RegExp.MethodSignatureArea));
		}
	}
	...
	public void StaticMethodSignature_true() {
		...
		for (int i = 0; i < expectedTrue.length; i++) {
			assertTrue(expectedTrue[i],
					expectedTrue[i].matches(RegExp.StaticMethodSignature));
		}
	}
	...
	public void StaticMethodSignature_false() {
		...
		for (int i = 0; i < expectedFalse.length; i++) {
			assertFalse(expectedFalse[i],
					expectedFalse[i].matches(RegExp.StaticMethodSignature));
		}
	}
	...
	public void MethodSignature_Group() {
		...
		if (methodMatcher.find()) {
			...
			assertEquals("    Map<String,Object>  	   map  ",
